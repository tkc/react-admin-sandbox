...
import { user } from "./data.json.js.js";
...
class UserCard extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={`${styles.userCard} px-3 py-5 ${type.length > 0 ? `${styles.typed} bg-${type}` : ""}`}>
        <button type="button" className={styles.plusBtn}>
          Add
        </button>
        <Avatar src={user.avatar} border borderColor={`${type.length > 0 ? "white" : ""}`} size="90" />
